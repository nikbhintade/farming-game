...
contract Metadata is Ownable {
    ...
    function drawSVG(bool isCat, uint8[] memory traits)
        public
        view
        returns (string memory)
    {
        uint8 offset = isCat ? 4 : 0;
        ...
        return
            string(
                abi.encodePacked(
                    '<svg id="CatFish" width="100%" height="100%" version="1.1" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                    svgString,
                    "</svg>"
                )
            );
    ...
    function tokenMetadata(
        ...
        string memory svg = drawSVG(isCat, traits);
        ...
        string memory metadata = string(
            abi.encodePacked(
                ...
                base64(bytes(svg)),
                '", "attributes":',
                compileAttributes(isCat, traits, tokenId),
                "}"
            )
        );
